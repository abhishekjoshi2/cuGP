...
void get_cholesky(double *M, int n)
{
	...
	printf("Total time for get_cholesky (n = %d) = %lf\n", n, endtime - startime);	
	// Now checking!
...
void compute_chol_get_mul_and_det()
{
	...
	double tmi_start = CycleTimer::currentSeconds();
	get_inverse_by_tmi(K, N);
      	cudaThreadSynchronize();
	double tmi_end = CycleTimer::currentSeconds();

	printf("Total time for get_inverse_by_tmi (N = %d) = %lf\n", N, tmi_end - tmi_start);
  	
	dim3 blockDim(32,32);
        dim3 gridDim( upit(N, blockDim.x), upit(N, blockDim.y));
	
	double llmns_start = CycleTimer::currentSeconds();
        lowertriangular_matrixmultiply_noshare<<<gridDim, blockDim >>>(K, Kinv, N);
        cudaThreadSynchronize();
	double llmns_end = CycleTimer::currentSeconds();
	printf("Total time for lowertriangular_matrixmultiply_noshare = %lf\n", llmns_end - llmns_start);
...
double compute_log_likelihood()
{
	...
	printf("Get loglikelihood starts\n");
	double compute_log_likelihood_start, compute_log_likelihood_end;
	double compute_k_train_start, compute_k_train_end;
	double compute_chol_get_mul_and_det_start, compute_chol_get_mul_and_det_end;

	compute_log_likelihood_start = CycleTimer::currentSeconds();

	threads_per_block = 512;
	number_of_blocks = upit((N * N), threads_per_block);

	compute_k_train_start = CycleTimer::currentSeconds();
	compute_K_train<<<number_of_blocks, threads_per_block>>>(X, K, loghyper, N, DIM); // kernel
	cudaThreadSynchronize();
	compute_k_train_end = CycleTimer::currentSeconds();

	compute_chol_get_mul_and_det_start = CycleTimer::currentSeconds();
	compute_chol_get_mul_and_det(); // set of kernels
	compute_chol_get_mul_and_det_end = CycleTimer::currentSeconds();

	double llans = evaluate_and_get_log_likelihood(); // kernel, or can be clubbed somewhere
	// printf("The value of loglikelihood = %lf\n", llans);

	compute_log_likelihood_end = CycleTimer::currentSeconds();
	printf("Time for compute_k_train: %lf. DIM is %d\n", compute_k_train_end - compute_k_train_start, DIM);
	printf("Time for compute_chol_get_mul_and_det: %lf\n", compute_chol_get_mul_and_det_end - compute_chol_get_mul_and_det_start);

	printf("Total Time for get_log_likelihood: %lf\n", compute_log_likelihood_end - compute_log_likelihood_start);
	printf("\n");
	return llans; 
}
...
void compute_gradient_log_hyperparams(double *localhp_grad)
{
	...
	double compute_gradient_log_hp_start = CycleTimer::currentSeconds();
	...
	double compute_gradient_log_hp_end = CycleTimer::currentSeconds();

	printf("Total time for compute_gradient_log_hp = %lf\n\n", compute_gradient_log_hp_end - compute_gradient_log_hp_start);
}
